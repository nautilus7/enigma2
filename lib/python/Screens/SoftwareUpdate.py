...
class SoftwareUpdate(Screen, HelpableScreen, ProtectedScreen):
	...
	def checkTrafficLight(self):
		...
		try:
			...
			message = "TEST IMAGE FEED. FIX ME!"
	...
	def keyUpdate(self):
		self.opkg.removeCallback(self.opkgCallback)
		updateLimit = 200  # BoxInfo.getItem("UpdateLimit", 200)
		if self.packageCount <= updateLimit:
			...
			message = [
				...
				_("There is a risk that your receiver will not boot or may malfunction after such a large on-line update."),
...
class RunSoftwareUpdate(Screen, HelpableScreen):
			...
			if self.updateFlag:
				...
				if self.errorCount == 0:
					...
					if self.deselectCount:
						...
						self["update"].appendText("\n%s\n" % _("Deselected packages usually occur because those packaged are incompatible with existing packages. While this is mostly harmless it is possible that your receiver may experience issues."))
				else:
					error = _("Your receiver might be unusable now. Please consult the manual for further assistance before rebooting.")
	...
	def keyCancel(self):
		...
		if self.upgradeCount != 0 and self.errorCount == 0:
			self.session.openWithCallback(self.keyCancelCallback, MessageBox, _("Update completed. A reboot is strongly recommended. Do you want to reboot your receiver now?"))
		else:
			self.close()
