...
from enigma import eActionMap, eEPGCache, eServiceReference, eStreamServer, getBestPlayableServiceReference, iRecordableService, quitMainloop, setPreferredTuner
...
from Tools.Trashcan import TrashcanInstance
...
SID_code_states = SID_symbol_states.setdefault(SystemInfo["model"], (None, 0))
...
class RecordTimerEntry(TimerEntry):
	...
	def calculateFilename(self, name=None):
		...
		if not dirname:
			return None
		self.recordingFilename = getRecordingFilename(filename, dirname)
		self.log(0, "Recording filename calculated as '%s'." % self.recordingFilename)
	...
	def tryPrepare(self):
		if self.justplay:
			...
			if self.repeated:
				...
				if event_id is None:
					event_id = -1
			prep_res = self.record_service.prepare(self.recordingFilename + self.record_service.getFilenameExtension(), self.begin, self.end, event_id, name.replace("\n", " "), description.replace("\n", " "), " ".join(self.tags), bool(self.descramble), bool(self.record_ecm))
			if prep_res:
				if prep_res == -255:
		...
		if next_state == self.StatePrepared:
			...
			if self.tryPrepare():
				...
				if not self.justplay:
					open(self.recordingFilename + self.record_service.getFilenameExtension(), "w").close()
					# Give the trashcan a chance to clean up
					# Need try/except as trashcan may not exist
					# for a missed recording started at boot-time.
					try:
						TrashcanInstance.cleanIfIdle(self.recordingFilename)
					except Exception as e:
						print("[RecordTimer] Failed to call trashcan.cleanIfIdle()!")
			...
			if self.justplay:
				if Screens.Standby.inStandby:
					...
					if pip_zap:
						...
						if cur_ref_group and cur_ref_group != self.service_ref.ref and self.InfoBarInstance and hasattr(self.InfoBarInstance.session, "pipshown") and not Components.ParentalControl.parentalControl.isProtected(self.service_ref.ref):
							if self.InfoBarInstance.session.pipshown and hasattr(self.InfoBarInstance, "showPiP"):
								self.InfoBarInstance.showPiP()
				...
				try:
					TrashcanInstance.markDirty(self.recordingFilename)
				except:
					pass
	...
	def failureCB(self, answer=False, close_pip=True, ref=None):
		...
		if answer:
			...
			if close_pip and not self.first_try_prepare:
				if self.InfoBarInstance and hasattr(self.InfoBarInstance.session, "pipshown") and self.InfoBarInstance.session.pipshown:
					if hasattr(self.InfoBarInstance, "showPiP"):
						self.InfoBarInstance.showPiP()
				if hasattr(self.InfoBarInstance.session, "pip"):
					del self.InfoBarInstance.session.pip
					self.InfoBarInstance.session.pipshown = False
...
class RecordTimer(Timer):
	...
	def createTimer(self, timerDom):
		...
		tags = tags.split(" ") if tags and tags != "None" else None
